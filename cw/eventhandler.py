    ...
    def run(self):
        ...
        while True:
            ...
            if event.type == KEYDOWN:
                ...
                # 左クリックイベント
                if event.button == 1:
                    self.mouse_lbutton = True

                # 右クリックイベント
                elif event.button == 3:
                    self.mouse_rbutton = True

            elif event.type == MOUSEBUTTONUP:
                # 左クリックイベント
                if event.button == 1:
                    if self.mouse_lbutton:
                        self.lclick_event()
                        self.mouse_lbutton = False

                # 右クリックイベント
                elif event.button == 3:
                    if self.mouse_rbutton:
                        self.rclick_event()
                        self.mouse_rbutton = False
