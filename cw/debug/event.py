...
class EventList(wx.TreeCtrl):
    ...
    def __init__(self, parent, size, currentfpath):
        ...
        self.SetFont(cw.cwpy.rsrc.get_wxfont("gothic", size=cw.s(11)-1, weight=wx.NORMAL))
        self._showallcards = False
        self.imglist = wx.ImageList(cw.s(16), cw.s(16))
        imgidx_area = self.imglist.Add(cw.s(cw.cwpy.rsrc.debugs["AREA"]))
        imgidx_battle = self.imglist.Add(cw.s(cw.cwpy.rsrc.debugs["BATTLE"]))
        imgidx_package = self.imglist.Add(cw.s(cw.cwpy.rsrc.debugs["PACK"]))
        imgidx_skill = self.imglist.Add(cw.s(cw.cwpy.rsrc.debugs["EVT_GET_SKILL"]))
        imgidx_item = self.imglist.Add(cw.s(cw.cwpy.rsrc.debugs["EVT_GET_ITEM"]))
        imgidx_beast = self.imglist.Add(cw.s(cw.cwpy.rsrc.debugs["EVT_GET_BEAST"]))
        self.imgidx_menucard = self.imglist.Add(cw.s(cw.cwpy.rsrc.debugs["CARD"]))
        self.imgidx_event = self.imglist.Add(cw.s(cw.cwpy.rsrc.debugs["EVENT"]))
        self.imgidx_ignition = self.imglist.Add(cw.s(cw.cwpy.rsrc.debugs["IGNITION"]))
        self.imgidx_keycode = self.imglist.Add(cw.s(cw.cwpy.rsrc.debugs["KEYCODE"]))
        self.imgidx_round = self.imglist.Add(cw.s(cw.cwpy.rsrc.debugs["ROUND"]))
