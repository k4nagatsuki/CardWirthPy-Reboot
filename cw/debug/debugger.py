...
class Debugger(wx.Frame):
    def __init__(self, parent: "cw.frame.Frame") -> None:
        ...
        run_menu = wx.Menu()
        mb.Append(file_menu, "ファイル(&F)")
        mb.Append(edit_menu, "編集(&E)")
        mb.Append(scenario_menu, "シナリオ(&S)")
        ...
        scenario_menu.AppendSeparator()
        self.mi_initvars = wx.MenuItem(scenario_menu, ID_INIT_VARIABLES, "状態変数の初期化(&V)")
        ...
        self.tl_money = self.tb1.AddTool(
            ID_MONEY, "所持金", rsrc["MONEY"],
            shortHelp="所持金を変更します。")
        self.tl_card = self.tb1.AddTool(
            ID_CARD, "手札配布", rsrc["CARD"],
            shortHelp="手札カードを配布します。")
        self.tb1.AddSeparator()
        self.tb1.SetToolBitmapSize(wx.Size(cw.ppis(20), cw.ppis(20)))
        self.tl_member = self.tb1.AddTool(
            ID_MEMBER, "冒険者", rsrc["MEMBER"],
            shortHelp="冒険者の情報を編集します。")
        self.tl_coupon = self.tb1.AddTool(
            ID_COUPON, "経歴", rsrc["COUPON"],
            shortHelp="冒険者の経歴を編集します。")
        self.tl_status = self.tb1.AddTool(
            ID_STATUS, "状態", rsrc["STATUS"],
            shortHelp="冒険者の状態を編集します。")
        self.tl_recovery = self.tb1.AddTool(
            ID_RECOVERY, "全回復", rsrc["RECOVERY"],
            shortHelp="全冒険者を全回復させます。")
        self.tb1.Realize()
        ...
        self.tl_update = self.tb2.AddTool(
            ...
            shortHelp="最新の情報に更新します。")
        self.tb2.AddSeparator()
        self.tl_redisplay = self.tb2.AddTool(
            ID_REDISPLAY, "背景更新", rsrc["EVT_REDISPLAY"],
            shortHelp="背景を更新します。")
        self.tb2.AddSeparator()
        self.tl_friend = self.tb2.AddTool(
            ID_FRIEND, "同行者", rsrc["FRIEND"],
            shortHelp="同行者カードの取得・破棄を行います。")
        ...
        self.tl_round = self.tb2.AddTool(
            ...
            shortHelp="バトルラウンドを変更します。")
        self.tb2.AddSeparator()
        self.tb2.SetToolBitmapSize(wx.Size(cw.ppis(20), cw.ppis(20)))
        self.tl_save = self.tb2.AddTool(
            ID_SAVE, "セーブ", rsrc["SAVE"],
            shortHelp="状況を記録します。")
        self.tl_load = self.tb2.AddTool(
            ID_LOAD, "ロード", rsrc["LOAD"],
            shortHelp="状況を再現します。")
        self.tb2.AddSeparator()
        self.tl_initvars = self.tb2.AddTool(
            ID_INIT_VARIABLES, "状態変数の初期化", rsrc["INIT_VARIABLES"],
            shortHelp="全ての状態変数を初期値に戻します。")
        self.tb2.AddSeparator()
        self.tl_loadyado = self.tb2.AddTool(
            ID_LOAD_YADO, "最終セーブに戻す", rsrc["LOAD_YADO"],
            shortHelp="最後にセーブした状態に戻します。")
        self.tb2.AddSeparator()
        self.tl_break = self.tb2.AddTool(
            ID_BREAK, "シナリオ中断", rsrc["BREAK"],
            shortHelp="シナリオを中断して、冒険者の宿に戻ります。")
        self.tb2.AddSeparator()
        self.tl_editor = self.tb2.AddTool(
            ID_EDITOR, "エディタで開く", rsrc["EDITOR"],
            shortHelp="シナリオをエディタで開きます。")
        self.tb2.Realize()
        ...
        self.tl_startevent = self.tb_event.AddTool(
            ID_STARTEVENT, "イベントの実行", rsrc["EVENT"],
            shortHelp="イベントを選択して実行します。")
        self.tb_event.AddSeparator()
        self.tl_stepreturn = self.tb_event.AddTool(
            ID_STEPRETURN, "ステップリターン", rsrc["EVTCTRL_STEPRETURN"],
            shortHelp="イベントのサブルーチンを抜けます。")
        self.tl_stepover = self.tb_event.AddTool(
            ID_STEPOVER, "ステップオーバー", rsrc["EVTCTRL_STEPOVER"],
            shortHelp="イベントを1コンテントだけ実行します。サブルーチンには入りません。")
        self.tl_stepin = self.tb_event.AddTool(
            ID_STEPIN, "ステップイン", rsrc["EVTCTRL_STEPIN"],
            shortHelp="イベントを1コンテントだけ実行します。サブルーチンに入ります。")
        self.tb_event.AddSeparator()
        self.tl_pause = self.tb_event.AddCheckTool(
            ID_PAUSE, "イベント一時停止", rsrc["EVTCTRL_PAUSE"],
            shortHelp="イベントを一時停止します。")
        self.tl_stop = self.tb_event.AddTool(
            ID_STOP, "イベント強制終了", rsrc["EVTCTRL_STOP"],
            shortHelp="イベントを強制終了します。")
        self.tb_event.AddSeparator()
        self.tl_breakpoint = self.tb_event.AddTool(
            ID_BREAKPOINT, "ブレークポイントの切替", rsrc["BREAKPOINT"],
            shortHelp="ブレークポイントを設定、または解除します。")
        self.tl_clear_breakpoint = self.tb_event.AddTool(
            ID_CLEAR_BREAKPOINT, "ブレークポイントの整理", rsrc["CLEAR_BREAKPOINT"],
            shortHelp="シナリオごとのブレークポイントをクリアします。")
        self.tb_event.AddSeparator()
        self.tl_showstacktrace = self.tb_event.AddCheckTool(
            ID_SHOW_STACK_TRACE, "呼び出し履歴の表示", rsrc["STACK_TRACE"],
            shortHelp="呼び出し履歴を表示します。")
        self.tb_event.AddSeparator()
        self.sc_waittime = wx.SpinCtrl(
            self.tb_event, -1, "イベント待機時間", size=(cw.ppis(40), cw.ppis(20)))
        self.sc_waittime.SetRange(0, 99)
        self.sc_waittime.SetValue(0)
        st = wx.StaticText(self.tb_event, -1, "ウェイト")
        self.tb_event.AddControl(st)
        self.tb_event.AddControl(self.sc_waittime)
        st = wx.StaticText(self.tb_event, -1, " (1=0.1秒)")
        self.tb_event.AddControl(st)
        ...
        self.tl_battle = self.tb_area.AddTool(
            ID_BATTLE, "戦闘", rsrc["BATTLE"],
            shortHelp="バトルを選択して戦闘を開始します。")
        self.tl_pack = self.tb_area.AddTool(
            ID_PACK, "パッケージ", rsrc["PACK"],
            shortHelp="パッケージを選択してイベントを開始します。")
        ...
        self.tl_select = self.tb_select.AddTool(
            ID_SELECTION, "選択メンバ",
            rsrc["SELECTION"], shortHelp="選択中のキャラクターを変更します。")
        ...
        self.tl_showparty = self.tb_select.AddTool(
            ID_SHOW_PARTY, "パーティ出現",
            rsrc["EVT_SHOW_PARTY"], shortHelp="パーティを出現させます。")
        self.tl_hideparty = self.tb_select.AddTool(
            ID_HIDE_PARTY, "パーティ隠蔽",
            rsrc["EVT_HIDE_PARTY"], shortHelp="パーティを隠蔽します。")
        self.tb_select.Realize()
        self.tb_select.AddSeparator()
        self.tl_bgm = self.tb_select.AddTool(
            ID_BGM, "BGM変更",
            rsrc["EVT_PLAY_BGM"], shortHelp="BGMを変更します。")
        self.tb_select.Realize()
        ...
        self.tl_selectedcard = self.tb_selectedcard.AddTool(
            ...
            rsrc["CARD"], shortHelp="選択中のカードを変更します。")
        ...
        self.view_stacktrace: Optional[StackTraceView] = None
            ...
            def func() -> None:
                ...
                try:
                    if cw.cwpy.is_playingscenario():
                        ...
                        cw.cwpy.update_skin(os.path.basename(cw.cwpy.skindir), restartop=False, switch_skin=True)
    ...
    def OnMoneyTool(self, event: wx.CommandEvent) -> None:
        ...
        dlg = cw.dialog.edit.NumberEditDialog(self, "所持金の変更",
                                              party.money, 0, 9999999, 1000)
        cw.cwpy.frame.move_dlg(dlg)
        if dlg.ShowModal() == wx.ID_OK:
            def func(party: cw.data.Party, value: int) -> None:
                party.set_money(value - party.money, blink=True)
                cw.cwpy.add_lazydraw(clip=cw.cwpy.statusbar.rect)
            cw.cwpy.exec_func(func, party, dlg.value)
        dlg.Destroy()
    ...
    def OnEditorTool(self, event: wx.CommandEvent) -> None:
        ...
        if self.view_tree.selectionitem is not None:
            content: Optional[cw.data.CWPyElement] = self.view_tree.selectionitem.content
        ...
        if cw.cwpy.is_playingscenario() and not cw.cwpy.is_runningevent():
            ...
            if dlg.ShowModal() == wx.ID_OK:
                ...
                for index in dlg.GetSelections():
                    ...
                    fpath = cw.cwpy.sdata.get_infofpath(resid)
                    if fpath is None:
                        s: str = os.path.basename(seq[index][2])
                        s = s + " の読込に失敗しました。"
                        ...
                        for index in indices:
                            ...
                            if key in friendids:
                                ...
                                if e is not None:
                                    ...
                                    if cw.cwpy.is_battlestatus() and cw.cwpy.battle and cw.cwpy.battle.is_ready() and\
                                            fcard.is_active():
                ...
                def func(start: Callable[[], None]) -> None:
                    try:
                        ...
                        if cw.cwpy.is_battlestatus():
                            ...
                            cw.cwpy.battle.process_exception(ex)
                if dlg.start_event:
                    event = dlg.events.get_selectedevent()
                    assert event
                    cw.cwpy.exec_func(func, event.start)
                else:
    ...
    def refresh_breakpointtool(self) -> None:
        ...
        if self.mi_breakpoint.IsEnabled() != enable:
            self.mi_breakpoint.Enable(enable)
            self.tl_breakpoint.Enable(enable)
            self.tb_event.Realize()
    ...
    def refresh_clearbreakpointtool(self) -> None:
        ...
        if self.mi_clear_breakpoint.IsEnabled() != enable:
            self.mi_clear_breakpoint.Enable(enable)
            self.tl_clear_breakpoint.Enable(enable)
            self.tb_event.Realize()
    ...
    def _refresh_areaname(self, force: bool = False) -> None:
        ...
        if s == s2:
            self.st_area.SetToolTip("")
        else:
            self.st_area.SetToolTip(s)
        ...
        if force or cw.cwpy.is_battlestatus() != self.tl_area._battletool:
            if cw.cwpy.is_battlestatus():
                ...
                if self.mi_area.GetBitmap() != bmp:
                    ...
                    self.tl_area.SetNormalBitmap(bmp)
                    self.tl_area.SetShortHelp("戦闘を中断します。")
                ...
                if self.mi_area.GetBitmap() != bmp:
                    ...
                    self.tl_area.SetNormalBitmap(bmp)
                    self.tl_area.SetShortHelp("エリアを選択して場面を変更します。")
                    self.tl_area._battletool = False
                    self.tb_area.Realize()
    ...
    def refresh_selectedcardname(self) -> None:
        ...
        if s == s2:
            self.st_selectedcard.SetToolTip("")
        else:
            self.st_selectedcard.SetToolTip(s)
    ...
    def _refresh_tools(self) -> None:
        ...
        def func(self: Debugger) -> None:
            ...
            savedvariables = bool(cw.cwpy.ydata and cw.cwpy.ydata.saved_variables)
            savedjpdcimage = bool(cw.cwpy.ydata and cw.cwpy.ydata.savedjpdcimage)
            ...
            def func(self: Debugger) -> None:
                ...
                if self.view_tree.selectionitem and is_playingscenario:
...
class VariableListCtrl(wx.ListCtrl):
    ...
    def __init__(self, parent: Debugger) -> None:
        ...
        self.mi_initvars = wx.MenuItem(self.popup_menu, ID_INIT_VARIABLES, "状態変数の初期化(&V)")
        ...
        def func() -> None:
            ...
            for var, local, editable in self.list:
                if editable and var.value != var.defaultvalue:
                    if isinstance(var, cw.data.Flag):
                        var.set(var.defaultvalue, updatedebugger=False)
                    elif isinstance(var, cw.data.Step):
                        var.set(var.defaultvalue, updatedebugger=False)
                    elif isinstance(var, cw.data.Variant):
                        var.set(var.defaultvalue, updatedebugger=False)
                    else:
                        assert False
                    var.write_value()
    ...
    def OnDClick(self, event: wx.MouseEvent) -> None:
        ...
        if self.GetSelectedItemCount() == 1:
            ...
            if dlg.ShowModal() == wx.ID_OK:
                if isinstance(item, cw.data.Flag):
                    def func_flag(item: cw.data.Flag, local: bool, value: bool) -> None:
                        item.set(value)
                        item.redraw_cards()
                        item.write_value()
    ...
    def refresh_variablelist_impl(self, event: Optional[cw.event.Event] = None) -> None:
        ...
        def func(self, event: Optional[cw.event.Event]) -> None:
            ...
            def extend(data: Union[cw.data.SystemData, cw.event.Event], local: bool, editable: bool) -> None:
                variants = list(data.variants.values())
                cw.util.sort_by_attr(variants, "name")
                vlist.extend(map(lambda a: (a, local, editable), variants))
                steps = list(data.steps.values())
                cw.util.sort_by_attr(steps, "name")
                vlist.extend(map(lambda a: (a, local, editable), steps))
                flags = list(data.flags.values())
                cw.util.sort_by_attr(flags, "name")
                vlist.extend(map(lambda a: (a, local, editable), flags))
...
class EventView(wx.ScrolledWindow):
    def __init__(self, parent: Debugger) -> None:
        ...
        # 現在実行中のイベントツリーとイベント
        self.current_event: Optional[cw.event.Event] = None
        self.current_tree: Optional[Dict[str, cw.data.CWPyElement]] = None
        self.current_content: Optional[cw.data.CWPyElement] = None
        # 現在実行中のContent(item)
        self.activeitem: Optional[EventViewItem] = None
        # itemの辞書(keyはコンテントデータ)
        self.items: Dict[cw.data.CWPyElement, EventViewItem] = {}
        self.itemlist: List[EventViewItem] = []
        self.selectionitem: Optional[EventViewItem] = None
    ...
    def OnPaint(self, event: wx.PaintEvent) -> None:
        ...
        if not self.itemlist:
            dc.DrawText("ここに実行中のイベントツリーが表示されます。", self.leftbarwidth+cw.ppis(10), cw.ppis(5))
            hint = "ヒント:"
            ...
            ts = dc.GetTextExtent(hint)
            dc.DrawText("ダブルクリックかEnterキー押下で任意の", tx+ts[0]+cw.ppis(5), ty)
            dc.DrawText("イベントコンテントを実行できます。", tx+ts[0]+cw.ppis(5), ty+ts[1])
            return
        ...
        for i, item in enumerate(self.itemlist):
            if item.nextlen == 0:
                ...
                e_child = item.nextdata[-1]
                if e_child.tag == "ContentsLine":
                    e_child = e_child[0]
                if self.items[e_child].pos[1] < ytop:
                    continue
            ix, iy = item.pos
            ix -= xtop
            iy -= ytop
            ...
            if item.nextlen == 0:
                ...
                for e_child in item.nextdata:
                    if e_child.tag == "ContentsLine":
                        e_child = e_child[0]
                    child = self.items[e_child]
    ...
    def switch_breakpoint(self, item: Optional["EventViewItem"] = None) -> None:
        if not cw.cwpy.is_playingscenario():
            return
        if item is None:
    ...
    def set_activeitem(self, item: "EventViewItem") -> None:
    ...
    def _refresh_tree(self, nowrunning: Optional[cw.event.Event],
                      trees: Optional[Dict[str, cw.data.CWPyElement]]) -> None:
    ...
    def create_item(self, parentitem: Optional["EventViewItem"], contents: cw.data.CWPyElement, shiftx: int,
        ...
        def update_pos() -> Tuple[Optional[cw.data.CWPyElement], int, Tuple[int, int]]:
            if parentitem:
                parent: Optional[cw.data.CWPyElement] = parentitem.content
                x = parentitem.pos[0]
        ...
        if isline:
            contents_l: Sequence[cw.data.CWPyElement] = contents
        else:
            contents_l = (contents,)

        for i, content in enumerate(contents_l):
            parent, x, pos = update_pos()
            if isline and i+1 < len(contents_l):
                nextdata: Sequence[cw.data.CWPyElement] = (contents_l[i+1],)
            else:
                e = content.find("Contents")
                if e is None:
                    nextdata = ()
                else:
                    nextdata = e
            ...
            if not (isline and i+1 < len(contents_l)):
...
class EventViewItem(object):
    def __init__(self, parent: cw.data.CWPyElement, event: cw.event.Event, content: cw.data.CWPyElement,
        ...
        if self.parent is not None:
            parentcontent = cw.content.get_content(self.parent)
            if parentcontent:
                s = parentcontent.get_childname(self.content, event)
        else:
...
class StackTraceView(wx.ListCtrl, wx.lib.mixins.listctrl.ListCtrlAutoWidthMixin):
    def __init__(self, parent: Debugger) -> None:
        ...
        self.imgidx_package: int = self.imglist.Add(cw.cwpy.rsrc.debugs["PACK"])
        self.imgidx_card: int = self.imglist.Add(cw.cwpy.rsrc.debugs["CARD"])
        self.imgidx_event: int = self.imglist.Add(cw.cwpy.rsrc.debugs["EVENT"])
        self.imgidx_link_start: int = self.imglist.Add(cw.cwpy.rsrc.debugs["EVT_LINK_START"])
        self.imgidx_call_start: int = self.imglist.Add(cw.cwpy.rsrc.debugs["EVT_CALL_START"])
        self.imgidx_link_package: int = self.imglist.Add(cw.cwpy.rsrc.debugs["EVT_LINK_PACKAGE"])
        self.imgidx_call_package: int = self.imglist.Add(cw.cwpy.rsrc.debugs["EVT_CALL_PACKAGE"])
        self.imgidx_effect: int = self.imglist.Add(cw.cwpy.rsrc.debugs["EVT_EFFECT"])
        self.imgidx_skill: int = self.imglist.Add(cw.cwpy.rsrc.debugs["EVT_GET_SKILL"])
        self.imgidx_item: int = self.imglist.Add(cw.cwpy.rsrc.debugs["EVT_GET_ITEM"])
        self.imgidx_beast: int = self.imglist.Add(cw.cwpy.rsrc.debugs["EVT_GET_BEAST"])
        self.imgidx_contents: Dict[str, int] = {}
        self._has_curcontent = False
        self.SetImageList(self.imglist, wx.IMAGE_LIST_SMALL)
        self.InsertColumn(0, "呼び出し履歴")
    ...
    def refresh_stackinfo(self) -> None:
        ...
        def func(self: EventView) -> None:
            ...
            nowrunning = cw.cwpy.event.get_nowrunningevent() if self.enable_eventview() else None
            event = cw.cwpy.event.get_event() if self.enable_eventview() else None
            if event:
                e = event.cur_content
                ...
                e = None
            stackinfo = cw.cwpy.event.stackinfo[:cw.cwpy.event.stackinfo_len] if self.enable_eventview() else []
    ...
    def append_stackinfo_cwpy(self, item: Union[cw.event.Event,
        ...
        def func(self, item: Union[cw.event.Event, Tuple[cw.event.Event, Optional[cw.data.CWPyElement], int]]) -> None:
            ...
            if self._has_curcontent:
                ...
                self.list.append(data)
            self.InsertItem(index, name, icon)
