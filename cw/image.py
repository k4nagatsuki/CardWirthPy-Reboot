...
def fix_108byteheader(data):
    """GIMPで256インデックスカラーにして色空間を書き込んだ時に
    生成されるV4(108バイト)のINFOヘッダをwxWidgetsが扱えないので
    無理やり40バイトのINFOヘッダに書き換える。
    """
    if len(data) < 14 + 108:
        return data, True
    s = struct.unpack("<BBIhhIIIiHHiIIIIIBBBB", data[0:14+44])
    if s[0] != ord('B'):
        return data, True
    if s[1] != ord('M'):
        return data, True
    ...
    biClrUsed = s[15]
    _biClrImporant = s[16]
    # RGBの順序が入れ替わっている際にパレット情報がおかしくなる
    p1 = s[17]
    p2 = s[18]
    p3 = s[19]
    _p4 = s[20]
    if p1 not in (ord('R'), ord('G'), ord('B')):
        return data, True
    if p2 not in (ord('R'), ord('G'), ord('B')):
        return data, True
    if p3 not in (ord('R'), ord('G'), ord('B')):
        return data, True
    ...
    biSize = 40
    data2 = \
        data[:2] + si.pack(bfSize) + data[6:10] + si.pack(bfOffBits) + si.pack(biSize) + data[18:54]

    if p1 == ord('B') and p2 == ord('G') and p3 == ord('R'):
        return data2 + data[54+m:], False

    # パレットの色順序をカラーマスクの指定に応じて入れ替える(BGRの順にする)
    buf = io.BytesIO()
    buf.write(data2)
    pos = 54+m
    for i in range(0, biClrUsed):
        c1 = data[pos+0:pos+1]
        c2 = data[pos+1:pos+2]
        c3 = data[pos+2:pos+3]
        c4 = data[pos+3:pos+4]
        ...
        pos += 4
    buf.write(data[pos:])
    assert len(buf.getvalue()) == len(data)-m
