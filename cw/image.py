...
class ImageInfo(object):
    ...
    def calc_basecardposition(self, params: Tuple[int, int], noscale: bool = False, basecardtype: Optional[str] = None,
        ...
        """カードに配置した時の描画位置を返す。
        ベースとなる情報が無い時はpygame.rect.Rect(0, 0, imgwidth, imgheight)を返す。
        """
        (imgwidth, imgheight) = params

        def getsize(resname: str) -> Tuple[int, int]:
    ...
    def _calc_basecardposition_impl(self, imgwidth: int, imgheight: int, noscale: bool, basecardtype: Optional[str],
        ...
        if basecardtype == "LargeCard":
            ...
            return pygame.rect.Rect(0, 0, imgwidth, imgheight)
...
class CardImage(Image):
    ...
    def wxrect(self) -> pygame.rect.Rect:
        ...
        return pygame.rect.Rect(0, 0, wxsize[0], wxsize[1])
...
class CharacterCardImage(CardImage):
    def __init__(self,
                 ccard: Union["cw.sprite.card.PlayerCard", "cw.sprite.card.EnemyCard", "cw.sprite.card.FriendCard"],
    ...
    def update_scale(self) -> None:
        ...
        # rect
        self.rect = pygame.rect.Rect(cw.s(self._pos_noscale), cw.s((95, 130)))
    ...
    def set_levelimg(self, level: int) -> None:
        ...
        size = (w, h)
        self.levelimg = pygame.surface.Surface(size, pygame.SRCALPHA).convert_alpha()
    ...
    def update(self, card: "cw.sprite.card.CWPyCard", test_aptitude: Optional[cw.header.CardHeader] = None) -> None:
        ...
        if card.is_analyzable() and not card.is_unconscious():
            ...
            if cw.util.find_resource(fpath_guage, cw.M_IMG) and cw.util.find_resource(fpath_lifemask, cw.M_IMG):
                ...
                lifeimg = pygame.surface.Surface(guage.get_size()).convert_alpha()
                ...
                lifeimg = pygame.surface.Surface(guage.get_size()).convert()
    ...
    def update_statusimg(self, ccard: Union["cw.sprite.card.PlayerCard",
        ...
        for subimg in seq:
            ...
            if isinstance(subimg, pygame.surface.Surface):
                ...
                self.image.fill(subimg[0], pygame.rect.Rect(pos, subimg[1]))
                size = subimg[1]

            clip2 = pygame.rect.Rect(pos[0]+ccard.rect.left, pos[1]+ccard.rect.top, size[0], size[1])
            if clip:
                clip.union_ip(clip2)
            else:
                clip = clip2
...
def create_type2textcell(text: str, face: str, size: int, color: Tuple[int, int, int],
    ...
    img = pygame.surface.Surface(tcellsize).convert_alpha()
    img.fill((0, 0, 0, 0))
...
def get_textcellfont(size: int, face: str, color: Tuple[int, int, int], bold: bool, italic: bool,
                     uline: bool, vertical: bool, antialiased: bool) -> Tuple["cw.imageretouch.Font", int]:
    """テキストセル用のフォントを生成し、
    (font, lineheight)を返す。
    """
...
def create_colorcell(size: Tuple[int, int], color1: Tuple[int, int, int, int], gradient: str,
    ...
    """ブレンド前のカラーセルを生成し、
    pygame.surface.Surfaceのインスタンスを返す。
    size: セルのサイズ
    color1: 基本色
    gradient: グラデーション方向。
...
def smoothscale(surface: pygame.surface.Surface, size: Tuple[int, int], smoothing: bool = True,
    ...
    if surface.get_width() == 0 or surface.get_height() == 0:
        return pygame.surface.Surface(size).convert()
    ...
    if smoothing:
        ...
        if iscard and w < size[0] and h < size[1]:
            ...
            #        解消された場合は以下の処理を削除する。
            bmp = pygame.surface.Surface((w+1, h+1)).convert(24)
