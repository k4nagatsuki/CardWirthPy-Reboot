...
class Environment(base.CWBinaryBase):
    ...
    def __init__(self, parent: None, f: "cw.binary.cwfile.CWFile", yadodata: bool = False,
        ...
        self.play_bgm = f.boolean()  # BGM再生
        self.play_sound = f.boolean()  # 効果音再生
        ...
        if 10 <= self.dataversion_int:
            self.effect_getmoney = f.boolean()  # 所持金増減時に点滅させる
            self.clickjump = f.boolean()  # 右クリックで待機時間を飛ばす
            self.keep_levelmax = f.boolean()  # レベルを最大値に維持する
        if 11 <= self.dataversion_int:
            self.bgeffectatselmode = f.boolean()  # 選択モードでカーテンをかける
        else:
            self.bgeffectatselmode = True
        self.viewtype_poster = f.byte()  # 貼紙の表示条件
        self.bgcolor_message = f.dword()  # メッセージ背景濃度
        self.use_decofont = f.boolean()  # 装飾フォントの使用
