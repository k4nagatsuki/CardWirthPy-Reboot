...
class BranchContent(EventContentBase):
    ...
    def branch_cards(self, cardtype: str) -> int:
        """カード所持分岐。最初の所持者を選択する。
    ...
    def get_boolean_index(self, flag: bool) -> int:
    ...
    def get_compare_index(self, cmptype: int) -> int:
...
class BranchStepContent(BranchContent):
    ...
    def get_childname(self, child: cw.data.CWPyElement, event: cw.event.Event) -> str:
        ...
        if step is not None:
            ...
            if self.get_contentname(child) == "○":
                return "ステップ『%s』が『%s』以上" % (step.name, valuename)
            else:
                return "ステップ『%s』が『%s』未満" % (step.name, valuename)
...
def call_package(resid: int, call: bool) -> bool:
    """パッケージを実行する。
...
class EffectContent(EventContentBase):
    ...
    def action(self) -> int:
        """効果コンテント。"""
        target: Optional[Union[cw.character.Character, List[cw.character.Character], cw.header.CardHeader]]
        if self.targetm == "CardTarget":
            ...
            if cw.cwpy.event.get_inusecard():
                ...
                for card in e_effectevent.targets:
                    assert isinstance(card, cw.character.Character)
                    target.append(card)
        else:
        ...
        def apply(target: cw.sprite.card.CWPyCard) -> None:
            if isinstance(target, cw.character.Character):
                cw.cwpy.event.is_changestate = True
                ...
                try:
                    if self.ignite:
                        ...
                        if runevent:
                            ...
                            if not self.eff.check_enabledtarget(target, event=not self.ignite):
                                target.remove_coupon("＠効果対象")
                                return

                    is_dead = target.is_unconscious() or target.is_paralyze()
                    success = self.eff.apply(target, event=not self.ignite, selectedmember=selectedmember)
                    if self.ignite:
                        target.remove_coupon("＠効果対象")
...
def end_scenario(complete: bool) -> None:
    ...
    cw.cwpy.sdata.end(showdebuglog=True, complete=complete)
...
def is_addablecoupon(coupon: str) -> bool:
    """
    シナリオ側から操作可能なクーポンか。
    """
    if not coupon:
...
class TalkContent(EventContentBase):
    ...
    def get_selections_and_indexes(self) -> List[Tuple[int, str]]:
...
class TalkDialogContent(TalkContent):
    ...
    def get_dialogs(self) -> List[Tuple[List[str], str]]:
        dialogs = []
    ...
    def get_childname(self, child: cw.data.CWPyElement, event: cw.event.Event) -> str:
        ...
        if fromstep is not None and tostep is not None:
            if self.get_contentname(child) == ">":
                ...
                return "ステップ『%s』が『%s』と等しい" % (fromstep.name, tostep.name)
            else:
                return "ステップ『%s』が『%s』より小さい" % (fromstep.name, tostep.name)
    ...
    def get_childname(self, child: cw.data.CWPyElement, event: cw.event.Event) -> str:
        ...
        if fromflag is not None and toflag is not None:
            if self.get_contentname(child) == "○":
                ...
                return "フラグ『%s』が『%s』と異なる" % (fromflag.name, toflag.name)
...
class MoveBgImageContent(EventContentBase):
    def __init__(self, data: cw.data.CWPyElement, is_changestate: bool = True) -> None:
        EventContentBase.__init__(self, data, is_changestate=True)
        self.cellname = data.getattr(".", "cellname", "")
...
class PostEventContent(EventContentBase):
    ...
    def do_action(command: str, arg: Optional[str]) -> None:
        ...
        if command in methoddict:
            ...
            cw.cwpy.call_dlg("ERROR", text="コマンド「%s」は存在しません。" % command)
