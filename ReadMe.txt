...
一部(確認されたのは1件のみ)のビットマップファイルの読み込みで内部エラーが発生し、予測不能なトラブルが発生する事がわかっています。これはライブラリの不具合であり、対処が困難であるため、当面の対策として該当するファイルの読み込みを拒否しています。

当該ファイルを表示しようとした時は、「読込できないイメージです」と表示されます。
